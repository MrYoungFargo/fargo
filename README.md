...
<body>
    ...
    <script>
        ...
        function initThreeJS() {
            ...
            const sphereGeometry = new THREE.SphereGeometry(30, 32, 32); // Increased radius from 25 to 30
            const sphere = new THREE.Mesh(sphereGeometry, sphereMaterial);
            sphere.position.set(-70, -35, -100); // Adjusted position
            objectGroup.add(sphere);

            // Torus - Made even larger, now with a distinct color
            const torusGeometry = new THREE.TorusGeometry(35, 10, 16, 100); // Increased radius and tube
            const torus = new THREE.Mesh(torusGeometry, torusMaterial);
            torus.position.set(70, 40, -105); // Adjusted position
            objectGroup.add(torus);

            // Icosahedron - Made even larger, now with a distinct color
            const icosahedronGeometry = new THREE.IcosahedronGeometry(33); // Increased radius from 28 to 33
            const icosahedron = new THREE.Mesh(icosahedronGeometry, icosahedronMaterial);
            icosahedron.position.set(0, -50, -98); // Adjusted position
            objectGroup.add(icosahedron);

            // Cone - Made even larger, now with a distinct color
            const coneGeometry = new THREE.ConeGeometry(30, 60, 32); // Increased radius and height
            const cone = new THREE.Mesh(coneGeometry, coneMaterial);
            cone.position.set(-75, 45, -110); // Adjusted position
            objectGroup.add(cone);

            // Dodecahedron - Made even larger, now with a distinct color
            const dodecahedronGeometry = new THREE.DodecahedronGeometry(32); // Increased radius from 26 to 32
            const dodecahedron = new THREE.Mesh(dodecahedronGeometry, dodecahedronMaterial);
            dodecahedron.position.set(75, -45, -107); // Adjusted position
